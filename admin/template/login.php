<?php
    // aqui verificamos que el login tenga todos los campos
     if (empty($_POST['mail']) || empty($_POST['password'])) {
        // cookie de errores
        setcookie('login_error', 'Rellena todos los campos', time() + 3600, '/');
        ...
        if ($user) {
            ...
            if (password_verify($password, $user["password"])) {
                ...
                setcookie('login_error', 'La contraseña no coincide', time() + 3600, '/');
                header("Location: loginForm.php");
                exit();
            }
        } else {
            setcookie('login_error', 'El correo no coincide', time() + 3600, '/');
            header("Location: loginForm.php");
            exit();
        }
    }
