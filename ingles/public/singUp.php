<?php
    if(isset($_POST['owner'])){
        ...
        if (empty($name) OR empty($lastname) OR empty($mail) OR empty($phone) OR empty($password) OR empty($passwordRepeat)) {
         array_push($errors,"<div class='alert alert-danger'>All fields must be filled.</div>");
        }
        // validar los emails
        if(!filter_var($mail, FILTER_VALIDATE_EMAIL)){
            array_push($errors,"<div class='alert alert-danger'>Enter a valid email address.</div>");
        }
        // longitud de caracteres
        if (strlen($password)<8) {
            array_push($errors,"<div class='alert alert-danger'>Password must be at least 8 characters long.</div>");
        }
        // validacion de contrasena
        if ($password!==$passwordRepeat) {
            array_push($errors,"<div class='alert alert-danger'>Passwords do not match.</div>");
        }
        // no emails repetidos
        ...
        if ($rowCount>0) {
         array_push($errors,"<div class='alert alert-danger'>This email is already registered.</div>");
        }
        ...
        if (count($errors)>0) {
            ...
            if ($prepareStmt){
                ...
                setcookie('alert_message', 'Registered successfully', time() + 3600, '/');
